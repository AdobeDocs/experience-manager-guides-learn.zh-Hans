...
| 自訂輸出選項 | 描述 |
| --- | --- |
| 输出类型 | 您要產生的輸出型別。 若要使用自訂DITA-OT外掛程式產生輸出，請選擇自訂選項。 |
| 设置名称 | 為您正在建立的輸出設定提供描述性名稱。 例如，您可以指定 _內部客戶輸出_ 或 _一般使用者輸出_. |
...
| 文件名 | 指定要用來儲存輸出的檔案名稱。<br><br>**注意**：如果您未提供檔案名稱，則會使用DITA map的標題來產生最終輸出檔案名稱。 如果地圖沒有標題，則會使用DITA map的檔案名稱來命名最終輸出。 系統會使用系統中設定的規則來清除檔案名稱，以處理任何無效字元。 |
