...
- 通过展开元素，可以从所选文本中删除元素标记并将其与其父元素合并。 例如，如果您拥有 `p` 中的元素 `note` 元素，您可以将 `p` 元素直接将文本合并到 `note` 元素。 在主题痕迹导航的上下文菜单中，可以使用“取消环绕元素”选项。 要取消环绕某个元素，请从主题的痕迹导航中选择该元素，右键单击该元素以打开上下文菜单，最后选择“取消环绕元素”以移除该元素，并将该元素的文本与其父元素合并。
