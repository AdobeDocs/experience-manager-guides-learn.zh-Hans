...
1. 登录AEM实例 *\（作为管理员\）* 并导航到CRX包管理器。 访问包管理器的默认URL为
   ...
   包管理器可管理本地AEM安装中的包。 有关使用包管理器的更多信息，请参阅 [如何使用包](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/content/implementing/developer-tools/package-manager.html?lang=en) 在AEM文档中。
...
**问题**  — 如果在Oxygon XML Author中未看到AEM Guides面板，请尝试以下解决方案：

解决方案1：

1. 在氧气XML创作中，启用插件。

   单击 **选项** \> **首选项** \> **插件** 并选择 **Adobe Experience Manager Guides的氧气插件。**
...
1. 如果您仍然看不到AEM Guides面板，请启用AEM Guides窗口。

   在氧气XML创作中，单击 **窗口** \> **显示视图** \> **AEM指南**.
...
插件现在使用文件名aem-pluginX.log \(*其中X表示旋转数*\)。
