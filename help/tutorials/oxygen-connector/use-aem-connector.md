...
2. 单击 **下一个** 并浏览到可用Oxygon XML Author的.exe文件的位置。
...
3. 在Oxygon XML Author的plugins文件夹中复制aem-connector-x.x文件夹。
4. 双击aem-connector-x.x-setup文件以启动安装程序。

5. 启动Oxygon XML Author。
...
### 安装用于从AEM Web界面启用文档编辑功能的软件包 {#id182CE0Q0TY4 .section}
...
1. 从您的IT团队获取包的.zip文件。
2. 登录AEM实例 *\（作为管理员\）* 并导航到CRX包管理器。 访问包管理器的默认URL为
...
3. 要上传氧气包，请单击 **上传包**.
4. 在上传包对话框中，导航到您在步骤1中下载的Oxygon包文件，然后单击“确定”。
...
### 配置属性自定义的首选项 {#id1827K0D0OHT .section}
...
2. 在 **文档类型关联** 选项卡，选择 **DITA**，然后单击 **扩展**.

   ![](images/document_type_association.png)

3. 在 **类路径** 选项卡，选择中的com.adobe.o2.connector **使用插件中的ID为父类加载器** 下拉菜单。
...
1. 在Oxygon XML Author中，单击&#x200B;**选项** \> **首选项**.
2. 在 **文档类型关联** 选项卡，选择 **DITA映射**，然后单击 **扩展**.
3. 在 **类路径** 选项卡，选择中的com.adobe.o2.connector **使用插件中的ID为父类加载器** 下拉菜单。
4. 在 **扩展** 选项卡中，进行以下更改：
5. 
...
### 在氧气XML创作中打开文件 {#id195GHJ0A0UB .section}
...
### 签入文件 {#id182CF0J0FHS .section}

签入文件时，系统中的本地副本将存储在AEM存储库中，并且文件的锁定将被删除。 执行以下步骤以检入文件：

1. 通过单击 **文件** \> **保存**.

2. 右键单击已检出文件，然后从以下两个选项中进行选择：
...
### 上传文件和文件夹 {#id195HC03F03J .section}
...
### 添加或删除收藏夹 {#id195HC04405P .section}
...
### 查看文件的版本历史记录 {#id195GI000D5Q .section}
...
### 查看文件的元数据 {#id195GHN0H05C .section}
...
2. 输入搜索查询\(例如， `introduction`\)在AEM Guides的氧气插件搜索栏中。
3. 单击搜索按钮或按Enter。
...
4. \（可选\）双击搜索结果中的文件以在Oxygen XML Author中打开它。
5. 要返回到“AEM资料档案库”视图，请执行下列操作之一：
...
2. 选择氧气XML作者 *&lt;version>* 在 **启动应用程序** 消息框。 您可以选择 **记住我选择的AEM链接** 选项以保存您的首选项。
...
3. 选择氧气XML作者 *&lt;version>* 在 **启动应用程序** 消息框。 您可以选择 **记住我选择的AEM链接** 选项以保存您的首选项。
4. 在“氧气XML创作”中编辑主题。
5. 查看AEM Guides的氧气插件中的主题。
...
2. 选择要应用条件属性的内容部分。
3. 双击Oxygon XML Author的“属性”面板中的条件属性。

   ![](images/attribute-panel.png)

4. 在 **可用** 编辑属性对话框的列，选择属性并单击 **添加**.
...
### “缺少AEM Guides”面板 {#id192BH200ZAX .section}
