...
该服务正在使用Adobe的尖端云解决方案（如App Builder、IO Eventing、IMS）来创建无服务器产品。 这些服务本身基于广泛接受的行业标准，比如Kubernetes，医生。

对新发布微服务的每个请求都在隔离的Docker容器中执行，该容器一次只运行一个发布请求。 系统会自动创建多个新容器，以备收到新的发布请求。 这种按请求配置的单个容器使微服务能够为客户提供最佳性能，而不会带来任何安全风险。 发布结束后，这些容器将被丢弃，从而释放所有已使用的资源。
...
此部分显示微服务的性能编号。 请注意，由于旧云架构在发布大型映射或执行多个并发发布时遇到问题，因此，此部分将比较Microservice的性能编号与AEM Guides on-prem产品。

如果您要在内部发布大型映射，则可能必须调整Java堆参数，否则您可能会遇到内存不足错误。 在云上，已对微服务进行了配置，并且具有开箱即用的最佳Java堆和其他配置。
...
* 内部部署
   ...
   在内部运行并发发布会导致性能严重下降。 如果出版商同时发布更多地图，则性能下降会更严重。

   <img src="assets/onprem_bulk_publish.png" alt="项目选项卡" width="600">

## 其他优势

必须在AEM实例上运行每个发布请求的某些路径，以获取要发送到Microservice的正确发布内容。 新的云架构使用AEM作业代替AEM工作流，与旧架构中的情况类似。 通过这项更改，AEM指南管理员可以单独配置云发布队列设置，而不会影响其他AEM作业或工作流配置。
