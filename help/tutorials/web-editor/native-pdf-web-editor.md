...
使用元数据选项卡可设置元数据字段，如作者的姓名、文档标题、关键字、版权信息以及PDF输出的其他数据字段。 您还可以为PDF输出添加自定义元数据。

此元数据会映射到输出PDF“文档属性”的“描述”选项卡中的元数据。
...
1. 在 **输出** ，单击 **预设** 查看“预设”面板。
要创建或配置输出预设，请参阅 [创建PDF输出预设](#create-output-preset).
1. 要保存设置，请单击 **全部保存** ![全部保存](assets/SaveFloppy_icon.svg) 图标。
1. 单击 **生成预设** ![生成预设](assets/generate-output.svg) 图标。
您可以在“输出预设”面板中查看选定输出预设旁边的进度条。
1. 输出生成完成后，单击  **查看输出** ![查看输出](assets/view-output.svg) 图标来查看输出。\
   A **成功** 对话框。
如果输出失败，将显示以下错误消息。
