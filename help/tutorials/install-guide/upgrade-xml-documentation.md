...
在运行进程之前，必须完成某些任务。 以下子部分将指导您完成先决条件、报告生成和迁移过程。 此外，安装AEM Guides 4.0版后，您可以根据客户设置自定义各种配置。

>[!NOTE]
>
> 此升级过程仅适用于版本3.8.5到版本4.0。有关从版本3.4或更高版本升级到3.8.5的过程，请参阅 *升级AEM指南* 特定产品的安装指南中的部分，请参见 [“帮助存档”页](https://helpx.adobe.com/xml-documentation-for-experience-manager/archive.html).
...
1. 已关闭所有活动审核。
1. 已关闭所有翻译任务。
1. 卸载AEM Guides早期版本\（主要版本或修补程序版本\）顶部安装的所有AEM Guides修补程序。

**安装版本4.0之前**

在安装版本4.0之前，请执行以下步骤：

1. 确保此时使用AEM Guides版本3.8.5。
...
| 预期响应 |  — 如果可以移动所有需要的节点，您将获得一个通过检查。 <br> — 如果目标位置中存在节点，您将收到相关错误。 清理存储库\（删除节点/var/dxml\）并重新安装升级包，然后再次触发此端点。 <br>**注意：** 这不是常见的错误，因为3.x AEM Guides早期未使用目标位置。 <br>  — 如果此脚本不成功，请不要继续，并报告给您的客户成功团队。 |
...
安装AEM Guides后，您可以将适用于从新安装的版本到安装的各种配置合并到一起。

>[!NOTE]
>
> 可以自定义dam-update-asset模型。 因此，如果已完成任何自定义设置，则需要将自定义设置和AEM Guides同步到模型的工作副本中。
...
1. 单击 **插入组件** \(作为流程的最后一步，负责AEM Guides的后处理\)。
1. 配置 **流程步骤** ，详情如下：
...
升级到版本4.3.0取决于AEM Guides的当前版本。 如果您使用的是版本4.2或4.2.x ，则可以直接升级到版本4.3.0。
...
在开始AEM Guides 4.3.0升级过程之前，请确保您具有：

1. 已升级到AEM Guides版本4.2或4.2.x并完成了各自的安装步骤。
1. 已关闭所有翻译任务。
