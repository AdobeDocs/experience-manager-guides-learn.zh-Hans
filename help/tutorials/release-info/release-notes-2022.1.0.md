...
* 存储库面板和文件浏览对话框中的搜索过滤器已得到增强，可提供更多可进一步自定义的筛选选项。
...
* 添加了新配置，以允许用户保留或删除以下情况下要删除的主题的版本： **删除并创建** 选项在AEM站点输出生成中使用。
...
* 无论用户何时创建DITA文件，缺省文件名都会显示在小大小写中，以与Native AEM文件夹创建方案内联。
