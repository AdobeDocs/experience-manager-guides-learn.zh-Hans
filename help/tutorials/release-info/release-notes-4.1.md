...
本发行说明涵盖4.1.x版Adobe Experience Manager指南(以后称为 *AEM指南*)。

## 升级到最新版本

您可以轻松地将当前版本的AEM指南升级到版本4.1.2。在继续升级到版本4.1.2的AEM指南之前，必须考虑以下几点：
* 如果您使用的是版本4.1或4.1.x，则可以直接升级到版本4.1.2。
* 如果您使用的是版本4.0.x，则需要先升级到版本4.1或4.1.x，然后再升级到版本4.1.2。
...
本部分列出了AEM指南4.1.2版本支持的软件应用程序的兼容性矩阵。
...
* 选择所有文件夹配置文件时，将显示一个不可见的文件夹配置文件（错误）。 (10393)
* 当用户的时区与服务器时区不同时，基线创建不会选择最新版本。 (10336)
* 安装AEM指南4.1后，Control+F快捷键不会在资产控制台上打开浏览器搜索模式。 (10339)
* 对于引用文件夹的主题，出现基线创建错误。 (10383)
* “输出预设”选项卡会间歇性地展示空白屏幕，在某些情况下，会显示不可编辑的预设。 (10390)
* 密钥空间管理会导致异常和错误。 (10449)

### 解决方法的已知问题

* 在翻译期间导出的基线不会加载到编辑器的基线选项卡中。

   **解决方法**:使用DITA映射功能板的基线选项卡。
