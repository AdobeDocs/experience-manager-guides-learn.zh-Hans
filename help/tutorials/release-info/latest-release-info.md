---
title: AEM Guides版本
description: 最新AEM Guides发行版和必需AEM版本
...
[!DNL Adobe Experience Manager Guides] 是部署在AEM上的应用程序。 这是一个功能强大、企业级组件内容管理解决方案(CCMS)，可在Adobe Experience Manager中启用本机DITA支持，使AEM能够处理基于DITA的内容创建和交付。
...
|  | 非UUID版本 | UUID版本 |
|---|---|---|
| **资产识别** | 所有资源都使用资源在存储库中的路径进行标识。 | 所有资产均使用其UUID（首次上传资产时系统生成的唯一ID）进行标识。 |
| **引用创建** | 所有内容引用都是根据其路径创建的。 | 所有内容引用都是基于其UUID创建的。 |

### UUID构建的优点

* UUID安装的性能更高：
   * 引用是独立于路径的：当引用是基于UUID而不是路径创建时，引用管理系统可感知到链接。
   * 移动/更新操作效率高：即使资产移动到存储库中的其他路径，UUID也保持不变。 因此，无需进行任何处理即可修补移动/更新操作中资源之间的引用。
...
* 如果您是现有客户，则可以选择移至UUID内部版本，因为现在可以从非UUID内部版本迁移到UUID内部版本。 欲知更多详情，请参见 *从Non-UUID迁移到UUID内容* 中的部分 **安装和配置Adobe Experience Manager Guides。**

>[!NOTE]
>
>* 客户在首次设置时需要在UUID和非UUID模式之间做出选择（如果您需要帮助，请联系客户成功经理，帮助您根据用例做出决定）。
>* 从AEM Guides的一个版本升级到较新版本时，客户将需要确保选择相同的模式（UUID/非UUID）以匹配其现有模式。 非UUID内部版本不应直接升级到UUID内部版本。 从非UUID内部版本迁移到UUID内部版本将需要内容迁移。
